...
export default function InputTextMask(props: InputHTMLAttributes<HTMLInputElement> & { mask?: string, replacement?: string | Replacement, onMasks?: MaskEventHandler | undefined, error?: boolean, erroMessage?: string }) {
    ...
    return (
        <div className="relative flex flex-col">
            <InputMask {...props} ref={inputRef} className={`peer font-poppins px-2 rounded-md w-full h-10 text-sm text-cinza-escuro ${props?.error ? 'border-error': 'border-cinza'} border
            ...
            <label className={`absolute left-2 top-2 font-poppins pointer-events-none text-cinza-escuro ${inputRef.current?.value ? 'invisible' : 'visible'}`}>{props?.title}</label>
            {props?.error && !focus && (
