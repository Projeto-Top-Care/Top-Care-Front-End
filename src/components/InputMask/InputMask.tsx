...
export default function InputText(props: InputHTMLAttributes<HTMLInputElement> & { mask?: string, replacement?: string | Replacement, onMasks?: MaskEventHandler | undefined, error?: boolean, message?: string }) {
    return (
        <div className="relative">
            ...
            focus:bg-branco bg-branco ${props.title ? 'placeholder-transparent' : 'placeholder-cinza'} focus:outline-none`} mask={props.mask} replacement={props.replacement} onMask={props.onMasks} />
                {
                    props.error && (
                        <span className="absolute mt-10 font-poppins text-sm text-error">{props.message}</span>
                    )
                }
            </div>
