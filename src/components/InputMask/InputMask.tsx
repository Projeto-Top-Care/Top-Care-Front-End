...
export default function InputText(props : InputHTMLAttributes<HTMLInputElement> & {mask?: string, replacement?: string | Replacement}) {
    return (
        <div className="relative">
            <InputMask {...props} className={`peer font-poppins px-2 rounded-md w-full h-10 text-sm text-cinza-escuro ${props?.checked ? 'border-error': 'border-cinza'} border
            focus:bg-branco bg-branco ${props.title? 'placeholder-transparent' : 'placeholder-cinza'} focus:outline-none`} mask={props.mask} replacement={props.replacement}/>
